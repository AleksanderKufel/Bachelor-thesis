...
*/
    rhoEFlux =
        0.5*( qLeft*(rhoELeft+pLeft) + qRight*(rhoERight+pRight) - lambdaMax*(rhoERight-rhoELeft+pRight-pLeft))*magSf;
	
	
    forAll(rhoScalarFlux,i)
    {
	rhoScalarFlux[i] = 0.5*( qLeft*rhoScalarLeft[i] + qRight*rhoScalarRight[i] - lambdaMax*(rhoScalarRight[i]-rhoScalarLeft[i]))*magSf; // 0.23

	//rhoScalarFlux[i] = 0.5*( qLeft*rhoScalarLeft[i] + qRight*rhoScalarRight[i])*magSf; // 0.095
	
	//rhoScalarFlux[i] = 0.5*( qLeft*rhoScalarLeft[i] + qRight*rhoScalarRight[i] - lambdaMax*(rhoRight-rhoLeft)*0.5*(rhoScalarLeft[i]/max(rhoLeft,1e-15)+rhoScalarRight[i]/max(rhoRight,1e-15)))*magSf; // 0.085
	
	//rhoScalarFlux[i] = rhoFlux * 0.5 * (rhoScalarLeft[i]+rhoScalarRight[i]);  // 0.24
	
	//rhoFlux > 0 ? rhoScalarFlux[i] = rhoFlux * rhoScalarLeft[i] : rhoScalarFlux[i] = rhoFlux * rhoScalarRight[i]; // 0.41
    }
