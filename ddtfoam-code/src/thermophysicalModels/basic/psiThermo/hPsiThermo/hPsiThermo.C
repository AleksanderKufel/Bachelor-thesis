...
// calculate the enthalpy that the cell would have if it had a different temperature Tgiven
/*
template<class MixtureType>
scalar Foam::hPsiThermo<MixtureType>::h
(
    const scalar& Tgiven,
    const label& cell
) const
{
    //tmp<scalar> th(new scalar());
    //scalar& h = th();
    //scalar h = this->cellMixture(cell).H(Tgiven);
    //return h;
    return this->cellMixture(cell).H(Tgiven);
}
*/
/*
template<class MixtureType>
//Foam::tmp<Foam::scalar> Foam::hPsiThermo<MixtureType>::h
Foam::scalar Foam::hPsiThermo<MixtureType>::h
(
    const scalar& Tgiven,
    const label& cell
) const
