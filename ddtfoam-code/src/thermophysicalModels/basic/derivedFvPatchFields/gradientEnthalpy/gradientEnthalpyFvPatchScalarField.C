...
/*
	...
	Info << "on patch " << patch().name() << ", the gradient is: " << nl << gradient() << endl;
	Info << "thermo.h(Tw, patchi) = " << thermo.h(Tw, patchi) << endl;
	Info << "thermo.h(Tw, patch().faceCells()) = " << thermo.h(Tw, patch().faceCells()) << endl;
	Info << "Tw.snGrad() = " << Tw.snGrad() << endl;
*/

	// test only:
        gradient() = thermo.Cp(Tw, patchi)*Tw.snGrad();
	Info << " fixed gradientEnthalpyFvPatchScalarField: on patch " << patch().name() << ", the enthalpy gradient is: " << gradient() << endl;
