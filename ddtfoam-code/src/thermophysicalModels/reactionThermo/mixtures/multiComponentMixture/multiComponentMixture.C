...
) const
{
    ...
    //Info << "multiComponentMixture: " << Y_[0][celli] << " / " << speciesData_[0].W() << " * " << nl << speciesData_[0] << endl;
    //Info << "mixture = " << nl << mixture_ << endl;
    for (label n=1; n<Y_.size(); n++)
    {
	...
	if(Y_[n][celli]) 
	{
	    Info << "multiComponentMixture: " << Y_[n][celli] << " / " << speciesData_[n].W() << " * " << nl << speciesData_[n] << endl;
	    Info << "mixture = " << nl << mixture_ << endl;
	}
	*/
    }
